...
class App:
    ...
    def load_file(self):
            path = filedialog.askopenfilename(title="Load brief from file", filetypes=[("Text files","*.txt"),("All files","*.*")])
            if path:
                try:
                    text = Path(path).read_text(encoding="utf-8")
                    self.brief.delete("1.0", END)
                    self.brief.insert("1.0", text)
                    self.status.set(f"Loaded brief from {Path(path).name}.")
    ...
    def generate(self):
            ...
            hpw_str = self.hpw_var.get().strip()
            brief_text = self.brief.get("1.0", END).strip()
            if not brief_text:
                messagebox.showwarning("Input needed", "Please enter a project brief or guidelines.")
                return
            try:
                due_date = parse_due(due_str)
            except:
                messagebox.showerror("Invalid date", "Please enter a valid due date in YYYY-MM-DD format.")
                return
            try:
                hours_per_week = float(hpw_str)
                if hours_per_week <= 0:
                    raise ValueError()
            except:
                messagebox.showerror("Invalid hours/week", "Please enter a positive number for hours per week.")
                return
            start_date = today_local()
            if due_date.date() <= start_date.date():
                messagebox.showerror("Invalid due date", "Due date must be in the future.")
                return

            prompt = make_prompt(title, due_str, hours_per_week, brief_text)
            self.status.set("Generating plan via Gemini AI…")
            self.root.update_idletasks()
            try:
                response = ai(prompt)
                tasks, assumptions = parse_tasks_json(response)
                if not tasks:
                    raise ValueError("No tasks parsed from AI response.")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to generate plan: {e}")
                self.status.set("Ready.")
                return

            events, csv_rows = schedule_tasks(tasks, start_date, due_date, hours_per_week)

            slug = slugify(title)
            timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
            ics_path = OUTDIR / f"{slug}-{timestamp}.ics"
            csv_path = OUTDIR / f"{slug}-{timestamp}.csv"
            try:
                write_ics(events, ics_path, cal_name=title)
                with csv_path.open("w", newline="", encoding="utf-8") as f:
                    writer = csv.writer(f)
                    writer.writerow(["Task","Hours","Start Date","End Date","Why"])
